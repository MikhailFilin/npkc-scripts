"""
concl_describe_time_up.py
Синхронизация данных describe_time_kodap из исходной ClickHouse в целевую.
Выборка: до 15000 самых свежих исследований на код КОДАП (НПКЦ), время описания ≤ 6 ч.
...
# OUTPUT: describe_time_kodap (target CH, Yandex Cloud)
"""
...
_BUFFER_PKL  = os.path.join(_SCRIPTS_DIR, 'temp_buffer_describe_time.pkl')
_TABLE_NAME  = 'describe_time_kodap'

_COLUMNS = [
    'diagnostic_code', 'diagnostic_name', 'accession_number',
    'organization', 'division', 'modality', 'device', 'body_part',
    'doctor_fio', 'describe_start', 'describe_end', 'duration_seconds',
    'load_datetime',
]
...
SELECT
    ...
    describe_start,
    describe_end,
    dateDiff('second', describe_start, describe_end) AS duration_seconds
FROM sampled
WHERE rn <= %(sample_size)s
ORDER BY diagnostic_code, describe_start DESC
"""
...
def connect_vpn():
    ...
    send_ntfy_alert("Запускаю VPN для describe_time...", title="VPN Connect", priority="default", tags="lock")
    try:
...
def _ensure_table_exists(client) -> None:
    client.command(f"""
        ...
        (
            ...
            body_part        String,
            doctor_fio       String,
            describe_start   DateTime,
            describe_end     DateTime,
            duration_seconds UInt32,
            load_datetime    DateTime
        )
        ENGINE = MergeTree()
        ORDER BY (diagnostic_code, accession_number)
...
def extract_phase() -> bool:
    ...
    print(f"[describe_time] extract_phase: запрос к source CH "
          f"(sample {SAMPLE_SIZE} / код, ≤ {MAX_DURATION_HOURS} ч)...")
    ...
    try:
        ...
        with open(_BUFFER_PKL, 'wb') as f:
            _pickle.dump(rows, f)
        print(f"[describe_time] extract_phase: сохранено {len(rows)} строк в буфер.")
        return True
    except Exception as e:
        print(f"[describe_time] extract_phase ошибка: {e}")
        if client_source:
            client_source.close()
        return False
...
def load_phase() -> bool:
    ...
    if not os.path.exists(_BUFFER_PKL):
        print("[describe_time] load_phase: буфер не найден")
        return False
    ...
    try:
        ...
        print(f"[describe_time] load_phase: ошибка чтения буфера: {e}")
        return False

    if not raw_rows:
        print("[describe_time] load_phase: буфер пуст, пропускаю.")
        return True

    client_target = None
    for attempt in range(1, 4):
        try:
            print(f"[describe_time] Подключаюсь к целевой ClickHouse (попытка {attempt})...")
    ...
    try:
        ...
        for row in raw_rows:
            ...
            if hasattr(d_start, 'tzinfo') and d_start.tzinfo is not None:
                d_start = d_start.replace(tzinfo=None)
            if hasattr(d_end, 'tzinfo') and d_end.tzinfo is not None:
                d_end = d_end.replace(tzinfo=None)
            ...
            processed_rows.append([
                ...
                d_start if d_start is not None else datetime(2000, 1, 1),
                d_end   if d_end   is not None else datetime(2000, 1, 1),
                int(dur_sec) if dur_sec is not None else 0,
                load_dt,
            ])

        print(f"[describe_time] load_phase: вставляю {len(processed_rows)} строк в {_TABLE_NAME}...")
        client_target.insert(_TABLE_NAME, processed_rows, column_names=_COLUMNS)
        print(f"[describe_time] load_phase: вставлено {len(processed_rows)} строк.")
        client_target.close()
        client_target = None
        return True
    except Exception as e:
        print(f"[describe_time] load_phase ошибка: {e}")
        if client_target:
            client_target.close()
...
def export_describe_time() -> bool:
    """Полный цикл: VPN → source CH → pickle → VPN off → target CH."""
    print(f"[describe_time] Начинаю синхронизацию (sample {SAMPLE_SIZE} / код, ≤ {MAX_DURATION_HOURS} ч)...")
    send_ntfy_alert(
        "Начинаю синхронизацию describe_time_kodap...",
        title="describe_time Start",
        priority="default",
        tags="inbox",
    )
    ...
    try:
        ...
        send_ntfy_alert(f"Сбой describe_time extract: {str(e)[:80]}",
                        title="describe_time Error", priority="urgent", tags="fire")
        try:
            disconnect_vpn()
        except Exception:
            pass
    ...
    if not extract_ok:
        send_ntfy_alert("Ошибка extract describe_time", title="describe_time Error",
    ...
    if result:
        send_ntfy_alert(
            "✅ describe_time_kodap обновлён.",
            title="describe_time Success",
            priority="high",
            tags="white_check_mark",
        )
    else:
        send_ntfy_alert(
            "❌ describe_time: ошибка загрузки в target CH",
            title="describe_time Error",
            priority="urgent",
            tags="warning",
        )
...
def main():
    print("Запуск concl_describe_time_up")
    send_ntfy_alert(
        "Запускаю concl_describe_time_up...",
        title="describe_time Start",
        priority="default",
        tags="robot",
    )

    success = export_describe_time()

    if success:
        send_ntfy_alert(
            "✅ Обновление describe_time_kodap завершено!",
        ...
        send_ntfy_alert(
            "❌ Обновление describe_time_kodap завершилось с ошибкой!",

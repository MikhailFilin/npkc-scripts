"""
minzdrav_kis_hours_2026h1.py
Рабочие часы КТ и МРТ-аппаратов стационаров (КИС) по неделям за I полугодие 2026 г.
Для предоставления в Минздрав.
...
Методология:
  ...
  3. Сумма по дням недели = часов за неделю
  4. Среднее по неделям = среднее часов в неделю за полугодие

INPUT:  kis.eris, dm_ap2.dct_lpu (source ClickHouse, через VPN)
OUTPUT: project/reports/minzdrav_kis_kt_mrt_hours_2026h1.xlsx
"""
...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, 'minzdrav_kis_kt_mrt_hours_2026h1.xlsx')
...
# Источник: kis.eris — ae_title, device_type, studyuid, run_dt (дата проведения)
# МО берём из dm_ap2.dct_lpu через facility_id
...
WITH daily AS (
    SELECT
        ...
        toYear(e.run_dt)             AS god,
        toWeek(e.run_dt, 3)          AS nedelya,
        toString(toMonday(e.run_dt)) AS date_nedeli,
        toDate(e.run_dt)             AS den,
        uniqExact(toStartOfHour(e.run_dt)) AS chasov_v_den,
        dateDiff('hour', toStartOfHour(min(e.run_dt)), toStartOfHour(max(e.run_dt))) + 1 AS span_den
    FROM kis.eris AS e
    ...
    WHERE toDate(e.run_dt) >= '{DATE_FROM}'
      ...
      AND e.device_type IN ('КТ', 'МРТ')
    GROUP BY
        tip_apparata, ae_title, mo_name, otdelenie,
...
SELECT
    ...
    round(
        avg(chasov_za_nedelyu) OVER (
            PARTITION BY tip_apparata, ae_title, mo_name, otdelenie
        ), 1
    ) AS srednee_chasov,
    span_za_nedelyu,
    round(
...
FROM weekly
ORDER BY god, nedelya, tip_apparata, ae_title, mo_name
"""
...
def format_xlsx(path: str, n_detail: int, n_summary: int):
    ...
    ws3.append(['Расчёт рабочих часов оборудования (КИС, стационары)'])
    ws3.append([])
    ws3.append(['1. Источник: kis.eris + dm_ap2.dct_lpu'])
    ws3.append(['2. Аппарат: ae_title, модальность: device_type'])
    ws3.append(['3. Дата проведения исследования: run_dt (DateTime из kis.eris)'])
    ws3.append(['4. run_dt → toStartOfHour() — округление до часа'])
    ws3.append(['5. Для каждого аппарата в каждый день — кол-во уникальных часовых слотов'])
    ws3.append(['   (= сколько разных часов хотя бы одно исследование было проведено)'])
    ws3.append(['6. Сумма часов по дням недели = часов за неделю'])
    ws3.append(['7. Среднее (часов в неделю) = среднее по всем неделям полугодия'])
    ws3.append([])
    ws3.append(['Период:', f'{DATE_FROM} — {DATE_TO}'])
    ws3.append(['Источник:', 'kis.eris + dm_ap2.dct_lpu'])
...
def main():
    print(f"Запуск: КИС часы КТ/МРТ за {DATE_FROM} — {DATE_TO}")
    ...
    df_summary['Итого_макс_мин']   = df_summary['Итого_макс_мин'].round(1)
